...
const tomorrow_condition = document.getElementsByClassName('tomorrow_condition');
...
async function getData(city = 'cairo') {
    ...
    let response = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=${key}&q=${city}&days=3`);
...
function display(data) {
    ...
    today_name.textContent = info.toLocaleDateString('en-us', { weekday: "long" });
    today_current_month.textContent = info.toLocaleDateString('en-us', { month: "long" });
    today_current_num.textContent = info.toLocaleDateString('en-us', { day: '2-digit' });
}
...
function displayNextDay(data) {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        nextDay_name[i].textContent = Day.toLocaleDateString('en-US', { weekday: "long" });
